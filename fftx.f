...
C
   50 CONTINUE
      RETURN
      END
corg  SUBROUTINE FFT991(A,WORK,TRIGS,IFAX,INC,JUMP,N,LOT,ISIGN)
...
cfj Change to print only RANK 0 PE >>
      if(myrank .eq. 0) then
      IF (IFAX(1) .LE. 0 ) PRINT *,'FFTFAX -- INVALID N'
      endif
cfj <<
c      CALL ULIBER(33, ' FFTFAX -- INVALID N', 20)
...
C THE FOLLOWING CALL IS FOR MONITORING LIBRARY USE AT NCAR
CVAX>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
CVAX  CALL Q8QST4 ( 4HXLIB, 6HCFFT99, 6HCFFT99, 10HVERSION 01)
CVAX>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
C
      NN = N+N
      INK=INC+INC
